...
actor KeyedReplicaObserversController<K: Hashable & Sendable, T: Sendable> {
    ...
    func setupObserverCounting(replica: any PhysicalReplica<T>) async {
        Task {
            for await event in await replica.eventStream.stream {
                if case .observerCountChanged(let observingState) = event {
                    print("⚡️ KeyedReplica", event)
                    ...
                    if replicaWithObserversCountDiff != 0 || replicaWithActiveObserversCountDiff != 0 {
                        ...
                        // TODO: в оригинальной реплике нет этого события, подумать
